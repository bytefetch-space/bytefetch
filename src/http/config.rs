...
impl HttpDownloadConfig {
    ...
    pub(super) fn set_throttle_speed(&self, throttle_speed: Option<u64>) {
        let count = self.tasks_count.max(1) as u64;
        let task_speed = throttle_speed.unwrap_or_default() / count;
